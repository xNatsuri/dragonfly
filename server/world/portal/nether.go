...
// Activate ...
func (n Nether) Activate(tx *world.Tx) {
	for _, pos := range n.Positions() {
		tx.SetBlock(pos, portal(n.axis), nil)
	}
}

// Deactivate ...
func (n Nether) Deactivate(tx *world.Tx) {
	for _, pos := range n.Positions() {
...
// Activated ...
func (n Nether) Activated(tx *world.Tx) bool {
	for _, pos := range n.Positions() {
		if tx.Block(pos) != portal(n.axis) {
			return false
		}
	}
	return true
