...
type TravelComputer struct {
	// Instantaneous determines if the entity given can travel instantly.
	Instantaneous bool
...
// DimensionalEntity represents a world.Entity that can travel between dimensions.
type DimensionalEntity interface {
	world.Entity
...
func (t *TravelComputer) TickTravelling(e DimensionalEntity, tx *world.Tx) {
	w := tx.World()
	box := e.H().Type().BBox(e).Translate(e.Position()).Grow(0.25)
	...
	case world.Nether:
		...
		if t.Instantaneous || (t.awaitingTravel && time.Since(t.start) >= time.Second*4) {
			t.mu.Unlock()
			t.Travel(e, tx, w.PortalDestination(world.Nether))
			t.mu.Lock()
...
// on the source world.
func (t *TravelComputer) Travel(e DimensionalEntity, source *world.Tx, destination *world.World) {
	sourceDimension, targetDimension := source.World().Dimension(), destination.Dimension()
	...
	go func() {
		destination.Exec(func(tx *world.Tx) {
			...
			tx.AddEntity(e.H())
			ent, ok := e.H().Entity(tx)
			if ok {
				ent.(DimensionalEntity).Teleport(spawn)
			}
